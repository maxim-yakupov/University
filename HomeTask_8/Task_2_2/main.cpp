...
int main()
{
    ...
    cleanUpPathMatrix(pathMatrix, vertices, 0);

    cout << endl;

    //unsigned int current = 0;
    //!!_____________________________
    dijkstra(matrix, pathMatrix, vertices, pathQueue, /*current*/0);
    ...
    while (!isQueueEmpty(printQueue, vertices))
    {
        ...
        cout << "town: " << current << " length of path between first & current: " << pathMatrix[current].lengthOfPath;
        printQueue[current] = false;
        //!!TRANSITIONAL_VERTICES
        if (pathMatrix[next].parent != current)
        {
            cout << "\n      went through";
            for (int i = pathMatrix[current].parent; i != 0; i = pathMatrix[i].parent)
            {
                cout << " " << i;
            }
        }
