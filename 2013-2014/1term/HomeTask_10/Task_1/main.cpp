...
long int memory = 0;
void writeMem(int size, const char str[]);

bool getInFileInformation(ifstream &infile);
bool openOutFile(fstream &outfile);
bool openCCOutFile(fstream &outfile);//

bool countFrequency(ifstream& infile, int* frequency, String &str);
void makeListOfLetters(List* queue, int* frequency, fstream &ccout);
void bubbleSort(List* list, int leftLimiter, int rightLimiter);
...
int main()
{
    ...
    fstream ccout;//
    cout << (openCCOutFile(ccout) ? "Result in file 'inf.txt'" : "Was created file 'inf.txt'");
    ...
    makeListOfLetters(queue, frequency, ccout);

    bubbleSort(queue, 1, queue->length);//quickSort(queue, 1, queue->length);
...
void mergeFirstNodes(List* queue)
{
    ...
    writeMem(sizeof(TreeNode), "TreeNode");//!!mem
...
bool openCCOutFile(fstream &outfile)
{
    outfile.open("inf.txt", ios_base::trunc);
    if (!outfile.is_open())//if don't exists
    {
        outfile.open("inf.txt", ios_base::in | ios_base::out | ios_base::trunc);//create
        return false;
    }
    else
    {
